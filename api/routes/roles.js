...
router.post('/add', auth.checkRoles("role_add"),async (req, res) => {
    ...
    try{
        ...
        if(!body.permissions || !Array.isArray(body.permissions) || body.permissions.length === 0){
            throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error", "At least one permission is required");
        }
...
router.post('/update', auth.checkRoles("role_update"),async (req, res) => {
    ...
    try{
        ...
        if(body.permissions && Array.isArray(body.permissions) && body.permissions.length > 0){
            ...
            let removedPermissions = permissions.filter(x => !body.permissions.includes(x.permission)); //body.permissions => ["category_view", "category_add"]
            let newPermissions = body.permissions.filter(x => !permissions.map(p => p.permission).includes(x)); //permissions => [{permission: "category_view"}, {permission: "category_update"}]
