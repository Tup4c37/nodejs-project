...
module.exports = function(){
    ...
    return {
        ...
        authenticate: function(){
            ...
            //istek içinde authorization header ı ile gönderilen jwt tokeni kontrol eder
        },
        checkRoles: (...expectedRoles) =>{
            return (req, res, next) => {

                let i = 0;
                let privileges = req.user.roles.filter(x => x).map(x => x.key);// req.user kısmı yukarıdaki done kısmından auth bi kullanıcıysa gelecek

                while(i < expectedRoles.length && !privileges.includes(expectedRoles[i])) i++;

                if(i >= expectedRoles.length){
                    let response = Response.errorResponse(new CustomError(HTTP_CODES.NOT_AUTHORIZED), "Need Permission","Need Permission" );
